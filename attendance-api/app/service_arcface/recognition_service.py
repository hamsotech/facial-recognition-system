"""
service_arcface/recognition_service.py — ArcFace Pipeline (PC / Server)
════════════════════════════════════════════════════════════════════════
Dùng chung: YOLO, MTCNN, Similarity, Attendance từ app.shared
Riêng biệt: ArcFace ONNX embedding (app.service_arcface.arcface_service)
"""
...
def start_live_attendance(session_id: str, registered_students: dict):
    """
    Chạy pipeline điểm danh realtime bằng ArcFace.

    Args:
        session_id:          UUID phiên học.
        registered_students: Dict { student_id: {"embedding": np.ndarray, ...} }

    NOTE: Hàm này dành cho chạy trực tiếp trên PC có màn hình.
          Để chạy trên Jetson không màn hình → dùng jetson_edge/pipeline.py
    """
    cap = cv2.VideoCapture(CAMERA_INDEX)
    if not cap.isOpened():
        logger.error("[ArcFace] Không thể mở camera.")
        return

    attended_set = set()
    last_snapshot_time = 0.0
    logger.info(f"[ArcFace] Bắt đầu điểm danh (Session: {session_id})")
...
if __name__ == "__main__":
    start_live_attendance("test_session_arcface", {})