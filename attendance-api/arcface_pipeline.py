...
from app.service_arcface.arcface_service import get_embedding
...
def load_registered_embeddings_db(conn) -> dict:
    """Tải ArcFace embeddings hợp lệ từ PostgreSQL."""
    sql = """
        SELECT
            s.id            AS student_id,
            s.full_name,
    ...
    print(f"[DB] Đã tải {len(registered)} sinh viên có ArcFace embedding.")
    return registered
...
# ══════════════════════════════════════════════════════════════════
# CHẾ ĐỘ CỤC BỘ (Local Mode — không cần DB)
# ══════════════════════════════════════════════════════════════════
def load_local_dataset(dataset_dir: str) -> dict:
    """Quét thư mục dataset để trích xuất ArcFace embeddings."""
    print(f"\n[Local Mode] Đang quét: {dataset_dir}")
    if not os.path.exists(dataset_dir):
        os.makedirs(dataset_dir, exist_ok=True)
        return {}

    cache_path = os.path.join(dataset_dir, "arcface_local_cache.pkl")
        ...
        for img_name in images:
            ...
            try:
                ...
                if face_tensor is not None:
                    ...
                    if emb is not None:
                        embs.append(emb if isinstance(emb, np.ndarray) else emb)
            except Exception as e:
...
def run_pipeline(session_id: str = None, is_local: bool = False, dataset_dir: str = "dataset"):
    ...
    registered = {}
    enrolled_ids = None
    session_info = {"class_code": "LOCAL", "subject_name": "ArcFace Cục Bộ"}

    # 1. Kết nối DB hoặc load local
    if is_local or session_id is None:
        print("[ArcFace] Chế độ CỤC BỘ.")
        registered = load_local_dataset(dataset_dir)
    else:
        print("[ArcFace] Chế độ POSTGRESQL.")
        try:
    ...
    print(f"  Model: ArcFace (ONNX) | Ngưỡng: {SIMILARITY_THRESHOLD} | Device: {DEVICE}")
    ...
    while True:
        ...
        if persons and (now - last_snap >= SNAPSHOT_COOLDOWN):
            ...
            query_emb = emb if isinstance(emb, np.ndarray) else emb
        ...
        cv2.putText(display, f"{session_info['class_code']} | ArcFace | Q=Thoat",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Hệ thống điểm danh ArcFace")
    ...
    parser.add_argument("--threshold", type=float, default=None,
                        help="Ngưỡng cosine similarity (0.0-1.0). Mặc định: ARCFACE_THRESHOLD trong .env hoặc 0.50")
